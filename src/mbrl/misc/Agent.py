...
print(env.action_space.shape, env.observation_space)
...
class Agent:
    ...
    def demon(self, horizon, policy):
        print('Demsonstration on the policy')
        env = gym.make('InvertedPendulumPyBulletEnv-v0')
        p.connect(p.DIRECT)
        env.render()
        obs, A, reward_sum, done = env.reset(), [], 0, False
