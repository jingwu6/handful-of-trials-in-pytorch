...
from src.mbrl.misc.Agent import Agent
...
class MBExperiment:
    ...
    def run_experiment(self, show_demon=False):
        """Perform experiment.
        """
        ...
        for i in range(self.ninit_rollouts):
            ...
            # obs size (1001,26); ac size(1000,6); rewards size (1000,1) ; here, 1000 is corresponding to the horizon
            # cartpole  (21, 5) (20, 1) (20,)
            traj_obs.append(samples[-1]["obs"])
            traj_acs.append(samples[-1]["ac"])
            traj_rews.append(samples[-1]["rewards"])
            # print('len', len(samples))
            # for sample in samples:
            #     print('iteration', sample["obs"].shape, sample["ac"].shape, sample["rewards"].shape)
        ...
        for i in trange(self.ntrain_iters):
            ...
            iter_dir = os.path.join(self.logdir, "train_ite r%d" % (i + 1))
